...
<body class="antialiased bg-gradient-to-br from-slate-50 via-neutral-50 to-slate-100 dark:from-neutral-900 dark:via-neutral-900 dark:to-neutral-950 min-h-screen">
    ...
    <!-- Hero -->
    <section class="bg-gradient-to-r from-orange-500 to-red-600 py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 class="text-4xl font-bold text-white mb-4">Bantuan Sosial</h2>
            <p class="text-xl text-orange-100">Informasi penyaluran bantuan sosial di desa</p>
        </div>
    </section>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="grid md:grid-cols-3 gap-6 mb-12">
            <div class="bg-white dark:bg-neutral-800 rounded-2xl p-6 shadow-lg border border-neutral-200/50 dark:border-neutral-700/50 text-center">
                <div class="text-4xl font-bold text-orange-600 mb-2">{{ \App\Models\Bansos::count() }}</div>
                <div class="text-neutral-600 dark:text-neutral-400">Total Penerima</div>
            </div>
            <div class="bg-white dark:bg-neutral-800 rounded-2xl p-6 shadow-lg border border-neutral-200/50 dark:border-neutral-700/50 text-center">
                <div class="text-4xl font-bold text-emerald-600 mb-2">{{ \App\Models\Bansos::where('status', 'Diterima')->count() }}</div>
                <div class="text-neutral-600 dark:text-neutral-400">Sudah Disalurkan</div>
            </div>
            <div class="bg-white dark:bg-neutral-800 rounded-2xl p-6 shadow-lg border border-neutral-200/50 dark:border-neutral-700/50 text-center">
                <div class="text-4xl font-bold text-blue-600 mb-2">{{ \App\Models\Bansos::distinct('jenis_bansos')->count('jenis_bansos') }}</div>
        ...
        <div class="bg-white dark:bg-neutral-800 rounded-2xl shadow-xl p-8 border border-neutral-200/50 dark:border-neutral-700/50">
            <h3 class="text-2xl font-bold text-neutral-900 dark:text-white mb-6">Daftar Penerima Bansos</h3>
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody>
                        @foreach(\App\Models\Bansos::take(10)->get() as $index => $bansos)
                        <tr class="border-b border-neutral-100 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-700/50">
                            ...
                            <td class="py-3 px-4 text-neutral-600 dark:text-neutral-400">{{ $bansos->jenis_bansos }}</td>
                            <td class="py-3 px-4 text-neutral-900 dark:text-white font-medium">Rp {{ number_format($bansos->jumlah, 0, ',', '.') }}</td>
                            <td class="py-3 px-4">
                                <span class="px-3 py-1 rounded-full text-xs font-semibold
                                    {{ $bansos->status == 'Diterima' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400' }}">
                                    {{ $bansos->status }}
                                </span>
                            </td>
                        </tr>
            ...
            <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-4">* Menampilkan 10 data teratas dari total {{ \App\Models\Bansos::count() }} penerima bansos</p>
        </div>
    </main>
